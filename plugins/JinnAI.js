...
if (conf.LANG == 'EN') vtalk_dsc = 'Starts to jimbrootan voice chat.', reply_eva = '*Reply to Any Voice Message!*'
...
Asena.addCommand({on: 'text', fromMe: wk, dontAddCommandList: true, deleteCommand: false}, (async (message, match) => {
    if (message.message.startsWith('jimbrootan') && conf.TALKING_JINN !== 'true') {        
        ...
        var finm = message.message.replace('jimbrootan', '').replace(' ', '')   
...
Asena.addCommand({on: 'text', fromMe: false, deleteCommand: false}, (async (message, match) => {
        if (conf.TALKING_JINN == 'true' && ((!message.jid.includes('-')) || (message.jid.includes('-') && 
...
var succ_off = ''
if (conf.LANG == 'TR') {
    fulleva_dsc = 'Tam fonksiyonel jimbrootan özelliklerini aktif eder. Hesabınızı bir chatbota dönüştürün!'
    already_on = 'jimbrootan yapay zekası halihazırda tüm fonksiyonları etkin.'
    already_off = 'jimbrootan yapay zekası halihazırda yarı fonksiyonel çalışıyor.'
    succ_on = 'jimbrootan, Tam Fonksiyonel Olarak Açıldı! Lütfen Biraz Bekleyin! ✅'
    succ_off = 'jimbrootan, Yarı Fonksiyonel Olarak Ayarlandı! Lütfen Biraz Bekleyin! ☑️'
}
if (conf.LANG == 'EN') {
    fulleva_dsc = 'Activates full functionaljimbrootan features. Turn your account into a ai chatbot!'
    already_on = 'jimbrootan artificial intelligence is already fully functional.'
    already_off = 'jimbrootan artificial intelligence is currently running semi-functional.'
    succ_on = 'jimbrootan Opened Fully Functionally! Please wait a bit! ✅'
    succ_off = 'jimbrootan Set to Semi-Functional! Please wait a bit! ☑️'
}
if (conf.LANG == 'ML') {
    fulleva_dsc = 'പൂർണ്ണമായും പ്രവർത്തനക്ഷമമായ jimbrootan സവിശേഷതകൾ സജീവമാക്കുന്നു. നിങ്ങളുടെ അക്കൗണ്ട് ഒരു ചാറ്റ്ബോട്ടാക്കി മാറ്റുക!'
    already_on = 'jimbrootan കൃത്രിമബുദ്ധി ഇതിനകം പൂർണ്ണമായി പ്രവർത്തിക്കുന്നു.'
    already_off = 'jimbrootan AI നിലവിൽ സെമി-ഫംഗ്ഷണൽ ആണ്.'
    succ_on = 'jimbrootan പൂർണ്ണമായും പ്രവർത്തനക്ഷമമായി തുറന്നു! കുറച്ച് കാത്തിരിക്കൂ! ✅'
    succ_off = 'സെമി-ഫങ്ഷണൽ ആയി jimbrootan സജ്ജമാക്കുക! കുറച്ച് കാത്തിരിക്കൂ! ☑️'
}

Asena.addCommand({ pattern: 'jimbrootan ?(.*)', desc: fulleva_dsc, fromMe: true,dontAddCommandList: true, usage: '.jimbrootan on / off' }, (async (message, match) => {
    var jimbrootan_status = `${conf.TALKING_JINN}`
    if (match[1] == 'on') {
        if (jimbrootan_status == 'true') {
